...
export const PROMOTION_ZONE_PLAYER_MAP = new Map<string, Player>();
(Object.keys(PROMOTION_ZONES) as Player[]).forEach(player => {
    PROMOTION_ZONES[player].forEach(coords => {
        PROMOTION_ZONE_PLAYER_MAP.set(`${coords.row}-${coords.col}`, player);
    });
});
